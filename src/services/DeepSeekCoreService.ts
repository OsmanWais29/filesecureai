...
export class DeepSeekCoreService {
  ...
  static async analyzeDocument(documentId: string): Promise<DeepSeekAnalysisResult | null> {
    try {
      ...
      await this.autoCategorizeBased OnAnalysis(documentId, analysis);
   ...
   */
  private static async autoCategorizeBased OnAnalysis(documentId: string, analysis: DeepSeekAnalysisResult) {
    try {
